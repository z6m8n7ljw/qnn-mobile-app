...
bool iotensor::IOTensor::deepCopyQnnTensorInfo(Qnn_Tensor_t *dst, const Qnn_Tensor_t *src) {
...
bool iotensor::IOTensor::copyTensorsInfo(const Qnn_Tensor_t *tensorsInfoSrc,
...
bool iotensor::IOTensor::copyGraphsInfoV1(const QnnSystemContext_GraphInfoV1_t *graphInfoSrc,
...
bool iotensor::IOTensor::copyGraphsInfo(const QnnSystemContext_GraphInfo_t *graphsInput,
...
bool iotensor::IOTensor::copyMetadataToGraphsInfo(const QnnSystemContext_BinaryInfo_t *binaryInfo,
                                          qnn_wrapper_api::GraphInfo_t **&graphsInfo,
  ...
  *tensors          = (Qnn_Tensor_t*)calloc(1, tensorCount * sizeof(Qnn_Tensor_t));
  ...
  for (size_t tensorIdx = 0; tensorIdx < tensorCount; tensorIdx++) {
    ...
    if (StatusCode::SUCCESS == returnStatus) {
      ...
      returnStatus =
          (iotensor::IOTensor::deepCopyQnnTensorInfo(((*tensors) + tensorIdx), &wrapperTensor) == true
               ? StatusCode::SUCCESS
               : StatusCode::FAILURE);
    }
    if (StatusCode::SUCCESS == returnStatus) {
